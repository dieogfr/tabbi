...
const initializeOptions = () => {
  const elements = Object.fromEntries(Object.keys(defaultSettings).map((key) => [key, document.getElementById(key)]));
...
const saveOptions = (elements) => {
  const options = Object.fromEntries(
    Object.entries(defaultSettings).map(([key, { type }]) => [
      key,
      type === "number" ? parseInt(getOptionValue(elements[key], type), 10) : getOptionValue(elements[key], type),
    ])
  );
...
const resetOptions = (elements) => {
  const defaultValues = Object.fromEntries(Object.entries(defaultSettings).map(([key, { value }]) => [key, value]));
